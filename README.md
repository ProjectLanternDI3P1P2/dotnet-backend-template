...
`PlayerDbContextFactory`; it is used as both the target and startup project for
EF Core Tools. This keeps `Player.Presentation` free of the EF Core Design
...
Creating a player publishes `player.created.v1`, whose payload is the
...
There are none yet, and `Player.Test` holds unit tests only —
...
7. Rename the template's `Combat.*` projects to your service name, and update
   `/k:` and `/o:` in `.github/workflows/sonar.yaml`.
